...
void BoundingBox::mergeBoxs(BoundingBox const &other) {
...
int BoundingBox::getSplitAxis() const {
	int dx = max[0] - min[0];
	int dy = max[1] - min[1];
	int dz = max[2] - min[2];
...
bool Geometry::intersect(const ray&r, isect&i) const
{
	...
	try {
		...
		cerr << "ERROR in Scene.cpp Geometry::intersect\n" <<  e.what() << endl;
	}
	return false;
}
...
void Scene::buildKdTree() {
	...
	for (size_t i = 0; i < objects.size(); ++i) {
		// If we encounter a trimesh, build a kdtree for it
		if (objects[i]->isTrimesh())
			objects[i]->buildKdTree();
	}
	kdtree = std::unique_ptr<KDNode<Geometry>>(new KDNode<Geometry>(objects));
	std::cout << "Just build the kdTree!" << std::endl;
}

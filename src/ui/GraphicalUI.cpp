...
void GraphicalUI::cb_aaSlides(Fl_Widget * o, void * v) {
	//std::cout << "AA was " << ((GraphicalUI*)(o->user_data()))->m_nAA << std::endl;
	((GraphicalUI*)(o->user_data()))->m_nAA =  int( ((Fl_Slider *)o)->value() );

	//std::cout << "AA is set to " << ((GraphicalUI*)(o->user_data()))->m_nAA << std::endl;
...
void GraphicalUI::cb_kdCheckButton(Fl_Widget * o, void * v){
	GraphicalUI* pUI = (GraphicalUI*)(o->user_data());
	pUI->m_kd = (((Fl_Check_Button*)o)->value() == 1);
}
...
void GraphicalUI::cb_render(Fl_Widget* o, void* v)
{
	...
	if (pUI->raytracer->sceneLoaded()) {
		...
		int aa = pUI->getAA();
		bool kd = pUI->getKD();
		pUI->raytracer->setKD(kd);;
		
		pUI->raytracer->traceSetup(width, height, aa, depth, kd);
...
GraphicalUI::GraphicalUI() {
	...
	m_mainWindow = new Fl_Window(100, 40, 350, 310, "Ray <Not Loaded>");
		...
		// install AA slider
		m_sizeSlider = new Fl_Value_Slider(10, 80, 180, 20, "AA Level");
		m_sizeSlider->user_data((void*)(this));	// record self to be used by static callback functions
		m_sizeSlider->type(FL_HOR_NICE_SLIDER);
		m_sizeSlider->labelfont(FL_COURIER);
		m_sizeSlider->labelsize(12);
		m_sizeSlider->minimum(1);
		m_sizeSlider->maximum(32);
		m_sizeSlider->step(1);
		m_sizeSlider->value(m_nAA);
		m_sizeSlider->align(FL_ALIGN_RIGHT);
		m_sizeSlider->callback(cb_aaSlides);
		...
		m_debuggingDisplayCheckButton = new Fl_Check_Button(0, 105, 180, 20, "Use KDTree");
		m_debuggingDisplayCheckButton->user_data((void*)(this));
		m_debuggingDisplayCheckButton->callback(cb_kdCheckButton);
		m_debuggingDisplayCheckButton->value(m_kd);
